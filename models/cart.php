...
class Cart
{
    ...
    public function sumOne($id)
    {
        try {
            $sql = "UPDATE `carrito` SET `unidades` = `unidades` + 1 WHERE `id_producto` = $id AND `email_cliente` = '{$this->getEmailCliente()}';";
            $this->pdo->query($sql);
            return "ok";
        } catch (Exception $e) {
            return ($e->getMessage());
    ...
    {
        try {
            $sql = "DELETE FROM carrito WHERE id_producto=$id AND email_cliente ='{$this->getEmailCliente()}'";
            $this->pdo->query($sql);
            return "ok";
        } catch (Exception $e) {
            return ($e->getMessage());
